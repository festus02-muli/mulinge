...
class MSFPluginOps_Ajax_Requests {
	...
	function _hooks(){
		...
		add_action( 'wp_ajax_smfb_getCkSequenceIds', array( $this, 'smfb_getCkSequenceIds' ) );

		add_action( 'wp_ajax_smfb_getConstantContactLists', array( $this, 'smfb_getConstantContactLists' ) );

		add_action( 'wp_ajax_sm_popb_enable_safe_mode', array( $this, 'sm_popb_enable_safe_mode' ) );
	...
	if( current_user_can('editor') || current_user_can('administrator') ) {
		...
		if ( ! wp_verify_nonce( $nonce, 'POPB_data_nonce' ) ) {
			...
			if($_SERVER['REQUEST_METHOD'] == 'GET') {
				...
				$generatedShortcode = $data['campaignPlacement']['generatedShortcode'];
				...
				update_post_meta($page_id, 'ULPB_Page_ShortCode',$generatedShortcode);
...
function smfb_formBuilderEmail_ajax(){
	...
	if ( 
	   ...
	   exit;
		...
		if ( sanitize_text_field($_POST['enteryourfirstnamehere']) ) {
			...
			$multiOptionFieldValues = explode(" ", sanitize_textarea_field($_POST['pbFormTargetInfo']) );
			...
			if (isset($formBuilder['widgetPbFbFormMailChimp']) && $is_MCextensionActive == true ) {
				...
				if ($formBuilderEnableMailChimp == 'true') {
					...
					*/
					...
					if ($widgetPbFbFormMailChimp['formBuilderMCDoubleOptin'] == '') {
						$dblOptin = 'subscribed';
					}
			...
			foreach ($_POST as $key => $value) {
				...
				if ($thisFieldName == 'psID' || $thisFieldName == 'pbFormTargetInfo' || $thisFieldName == 'POPB_Form_Nonce' || $thisFieldName == '_wp_http_referer' || $thisFieldName == 'enteryourfirstnamehere' || $thisFieldName == 'g-recaptcha-response') {
				}else{
					...
					if ($thisFormEmailOptions['formEmailFormat'] == 'plain') {
						...
						if (strtolower($thisFieldName) == 'email' || strtolower($thisFieldName) == 'enter email' || strtolower($thisFieldName) == 'your email' || strtolower($thisFieldName) == 'email here' || strtolower($thisFieldName) == 'email_address' || strtolower($thisFieldName) == 'email address' || strtolower($thisFieldName) == 'your email') {
							$submitterEmail = sanitize_text_field($value);
						}
						if (strtolower($thisFieldName) == 'name' || strtolower($thisFieldName) == 'first name' || strtolower($thisFieldName) == 'firstname') {
							$submitterName = sanitize_text_field($value);
						...
						if ( strtolower($thisFieldName) != 'email' && strtolower($thisFieldName) != 'name' && strtolower($thisFieldName) != 'enteryourfirstnamehere' ) { // customs fields without name & email keys.
			...
			$submissionPostMetaValue = array('Form_Name'=> $formNameText, 'Form_Fields' => $merge_vars, 'date' => $todaysDate, 'post_id' => $ssm_post_id );
			...
			$ulpb_recentEntries_saved = get_option( 'ulpb_formBuilder_subForm_recent_entries', $default = false );

			if ($ulpb_recentEntries_saved != false) {

				if (! is_array($ulpb_recentEntries_saved)) {
					$ulpb_recentEntries_saved = array();
				}

				array_push($ulpb_recentEntries_saved, $submissionPostMetaValue);
			...
			if ($widgetPbFbFormAllowDuplicates == 'false') {
				...
				if ($subExists == true) {
					...
					update_option( 'ulpb_formBuilder_subForm_recent_entries', $ulpb_recentEntries_saved );
...
function smfb_getCkSequenceIds(){
...
function sm_popb_enable_safe_mode(){
